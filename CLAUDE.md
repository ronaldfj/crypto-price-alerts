...
| `btc_level_monitor.html` | Dashboard visual manual (HTML/JS, sin build, abrir directo en el navegador) de niveles de precio para BTC marcados a mano desde un gráfico de TradingView. Independiente del resto del proyecto: no usa `data_source.py`/Bybit/OKX ni `alerts_state.db` — trae precio en vivo de CoinGecko/Kraken/Coinbase directo desde el navegador y persiste el historial en `localStorage`. Los niveles (`LEVELS`) están hardcodeados y no se recalculan; hay que editarlos a mano si cambia el gráfico de referencia, y en `btc_level_monitor.py` (abajo) si se quiere mantener la alerta automática sincronizada. Solo visual — no manda Telegram; para eso ver `btc_level_monitor.py` |
| `btc_level_monitor.py` | Alertas automáticas de cruce de nivel para BTC, misma lista `LEVELS` que el HTML de arriba (duplicada a mano, no hay fuente única). Corre dentro de `alert_production.yml` (mismo cron de 4h que `alert.py`) vía `fetch_latest_price("BTC")`, guarda el último precio en la tabla `meta` de `alerts_state.db` (`btc_level_monitor:last_price`) y manda por Telegram (reusa `send_telegram` de `alert.py`) cuando detecta un cruce real, con dirección (🟢 al alza / 🔴 a la baja). Independiente del resto del pipeline de scoring — no usa `evaluate_*`, no respeta el bloqueo de BTC en `market_context.json` (es solo cruce de nivel, no señal de entrada) |
