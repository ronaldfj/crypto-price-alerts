...
| `btc_level_monitor.html` | Monitor standalone (HTML/JS, sin build, abrir directo en el navegador) de niveles de precio para BTC marcados a mano desde un gráfico de TradingView. Independiente del resto del proyecto: no usa `data_source.py`/Bybit/OKX ni `alerts_state.db` — trae precio en vivo de CoinGecko/Kraken/Coinbase directo desde el navegador y persiste el historial en `localStorage`. Los niveles (`LEVELS`) están hardcodeados en el script y no se recalculan; hay que editarlos a mano si cambia el gráfico de referencia |
